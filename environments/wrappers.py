...
class ActionDiscretizeWrapper(PyEnvironmentBaseWrapper):
  ...
  def __init__(self, env, num_actions):
    ...
    **Note:** Only environments with a single BoundedArraySpec are supported.
    ...
    action_spec = self._flatten(env.action_space)
  ...
  def _pack_sequence_as(self, structure, flat_sequence):
    ...
    return torch.nn.utils.rnn.pad_packed_sequence(flat_sequence, batch_first=True)
  ...
  def is_nested(input_spec):
    if isinstance(input_spec, (list, tuple, dict)):
        return True
    return False
  ...
  def _step(self, action):
    ...
    if self.is_nested(env_action_spec):
      continuous_actions = self._pack_sequence_as(env_action_spec,
                                                    [continuous_actions])
...
class ActionClipWrapper(PyEnvironmentBaseWrapper):
  ...
  def _step(self, action: Tensor):
    ...
    def _clip_to_spec(act_spec, act):
        # PyTorch allows min and max to be None
        if act_spec.minimum is None and act_spec.maximum is None:
            return act
        return torch.clamp(act, min=act_spec.minimum, max=act_spec.maximum)

    def map_structure_up_to(structure, func, *rest):
        if not isinstance(structure, Iterable) or isinstance(structure, (str, bytes)):
            return func(*rest)
        return type(structure)(map_structure_up_to(s, func, *r) for s, r in zip(structure, zip(*rest)))

    clipped_actions = map_structure_up_to(env_action_spec, _clip_to_spec, env_action_spec, action)
  ...
  def __init__(self, env):
    ...
    def is_nested(input_spec):
      if isinstance(input_spec, (list, tuple, dict)):
          return True
      return False
    if is_nested(self._env.action_spec()):
...
class FlattenObservationsWrapper(PyEnvironmentBaseWrapper):
  ...
  def _flatten_nested_observations(self, observations, is_batched):
    ...
    flat_observations = [np_flatten(x) for x in self._flatten(observations)]
...
class HistoryWrapper(PyEnvironmentBaseWrapper):
  ...
  def _get_observation_spec(self):
      def _update_shape(spec):
          new_shape = (self._history_length,) + spec.shape
          return spec._replace(shape=new_shape)

      observation_spec = self._map_structure(_update_shape, self.env.observation_spec())

      if self._include_actions:
          action_spec = self._map_structure(_update_shape, self.env.action_spec())
  ...
  def _map_structure(self, func, structure):
      if isinstance(structure, dict):
          return {k: self._map_structure(func, v) for k, v in structure.items()}
      elif isinstance(structure, tuple) and hasattr(structure, '_fields'):  # namedtuple
          return type(structure)(*(self._map_structure(func, v) for v in structure))
      else:
  ...
  def _zeros_from_spec(self, spec):
    ...
    return self._map_structure(_zeros, spec)
