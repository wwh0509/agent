...
def unstack_actions(batched_actions):
  ...
  flattened_actions = [item for sublist in batched_actions for item in sublist]

  # Determine the structure of the original batch
  batch_size = len(batched_actions)
  structure = namedtuple('Action', ['action' + str(i) for i in range(len(flattened_actions) // batch_size)])

  # Unstack actions by reconstructing the nested structure
  unstacked_actions = [
      structure(*actions) for actions in zip(*[iter(flattened_actions)]*batch_size)
  ]
