...
from agent.gibson_extension.termination_conditions.MultiPointGoal import MultiPointGoal
...
from agent.gibson_extension.reward_functions.slack_reward import SlackReward
import json
...
class MultiPointNavTask(BaseTask):
    ...
    def reset_agent(self, env):
        ...
        #TODO: 修改逻辑，根据数据集中的数据进行初始化
        episode_idx = str(env.current_episode + 1)
        self.target_pos_list = np.array(self.episode_data[episode_idx][0])
    ...
    def get_termination(self, env, collision_links=[], action=None, info={}):
        ...
        """
        done, info = super(MultiPointNavTask, self).get_termination(
            env, collision_links, action, info)
        if info['success'] and self.finish_point + 1 < self.num_point:
            self.partly_success = True
            info['success'] = False
            info['done'] = False
            done = False
