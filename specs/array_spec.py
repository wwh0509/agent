...
def sample_bounded_spec(spec, rng):
    ...
    Returns:
        A np.array sample of the requested space.
    """
    low = spec.minimum
    high = spec.maximum

    if spec.dtype == np.float32 or spec.dtype == np.float64:
        ...
        if np.any(high < np.iinfo(spec.dtype).max):
            high = np.where(high < np.iinfo(spec.dtype).max, high + 1, high)
...
def sample_spec_nest(structure, rng, outer_dims=()):
    ...
    return torch.nested_tensor(map(sample_fn, structure))
...
def check_arrays_nest(arrays, spec):
    ...
    """
    # Check that arrays and spec have the same structure.
    try:
        torch.nested_tensor([arrays, spec])
    ...
    checks = torch.nested_tensor([check_array, spec, arrays])
    # Only return True if all the checks pass.
    return all(torch.flatten(checks))
...
def add_outer_dims_nest(structure, outer_dims):
    ...
    return torch.nested_tensor(map(add_outer_dims, structure))
...
class BoundedArraySpec(ArraySpec):
    ...
    def __init__(self, shape, dtype, minimum=None, maximum=None, name=None):
        ...
        low = np.iinfo(dtype).min if np.issubdtype(dtype, np.integer) else -np.inf
        high = np.iinfo(dtype).max if np.issubdtype(dtype, np.integer) else np.inf
