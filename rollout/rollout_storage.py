...
from agent.common.common import TensorDict
...
class RolloutStorage:
    ...
    def __init__(
        ...
        # 这个subgoal是在map上的position
        # self.buffers["subgoal"] = torch.zeros(
        #     numsteps + 1, num_envs, 2, dtype=torch.float32
        # )
        #
        # self.buffers["prev_subgoal"] = torch.zeros(
        #     numsteps + 1, num_envs, 2, dtype=torch.float32
        # )


        # self.buffers["goal_map_pos"] = torch.zeros(
        #     numsteps + 1, num_envs, 2, dtype=torch.float32
        # )

        # if action_space.__class__.__name__ == "ActionSpace":
        self.buffers["actions"] = self.buffers["actions"].float()
        self.buffers["prev_actions"] = self.buffers["prev_actions"].float()
